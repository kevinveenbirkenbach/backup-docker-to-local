...
do
  ...
  do
    ...
    docker run --rm --volumes-from "$docker_container_name" -v "$host_backup_folder_path:$docker_backup_folder_path" "kevinveenbirkenbach/alpine-rsync" rsync -a --delete "$rsync_source_path" "$rsync_docker_destination_path" || rmdir "$rsync_host_destination_path" && echo "skipped.";
  done
